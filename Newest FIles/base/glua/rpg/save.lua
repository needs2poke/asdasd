...
function RPG.Save.GetGuidKey(player)
    if not player or not player.IsValid or not player:IsValid() then
        return nil
    end
    local guid = player.GetGUID and player:GetGUID() or nil
...
function RPG.Save.BuildSnapshot(game)
    ...
    local snapshot = {
        ...
        savedAt = (Game and Game.GetTime and Game.GetTime()) or 0,  -- engine ms; os is sandboxed
        ...
        player = {
            ...
            forceSevered = p.forceSevered or false,    -- Phase 1 (#1): persisted Truth-ending Sever marker
...
function RPG.Save.Write(playerOrGame)
    ...
    local err
    ok, err = RPG.Save.WriteToPath(path, jsonStr)
    if ok then
        game.lastSaveAt = (Game and Game.GetTime and Game.GetTime()) or 0  -- engine ms; os is sandboxed
    end
    return ok, err
end
...
function RPG.Save.Read(player)
    ...
    if not content or content == "" then
        local guidKey = RPG.Save.GetGuidKey(player)
...
function RPG.Save.RestoreFromSnapshot(player, snapshot)
    ...
    game.player.forceSevered = sp.forceSevered or false    -- Phase 1 (#1): Truth-ending Sever marker
...
function RPG.Save.AutoSave(player)
    ...
    local nowMs = (Game and Game.GetTime and Game.GetTime()) or 0
    local last = game.lastSaveAt or 0
    -- Negative delta means level.time reset (map change / server restart); allow the save through.
    if last > 0 and (nowMs - last) >= 0 and (nowMs - last) < RPG.Config.AUTOSAVE_COOLDOWN_MS then
...
--- Side effect: may trigger GUID-to-account save migration (inherited from Save.Read).
--- Result: { class, level, currentRoom, currentRoomName, savedAt, playerName }
