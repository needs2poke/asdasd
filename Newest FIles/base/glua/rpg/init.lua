...
hook.Add("PlayerSay", "RPG.ChatCommand", function(playerNum, text, teamOnly)
    ...
    if args[1] == "!rpgdebug" or args[1] == "/rpgdebug" then
        ...
        if not sub then
            ...
            player:SendPrint("^7!rpgdebug save_status")
            return ""
        end
        ...
        if sub == "save_status" then
            ...
            if game and game.lastSaveAt then
                ...
                local age = nowMs - game.lastSaveAt
                if age < 0 then age = 0 end  -- map change / restart since save
                player:SendPrint("^7Last save:      ^3" .. math.floor(age / 1000) .. "s ago (engine time)")
            ...
            if meta then
                ...
                player:SendPrint("^7Snapshot meta:  ^3Level " .. meta.level .. " " ..
                    ...
                    " (savedAt=" .. tostring(meta.savedAt) .. " engine ms)")
            else
...
hook.Add("GameShutdown", "RPG.GameShutdown.AutoSaveAll", function()
    ...
    for clientNum, game in pairs(RPG.players or {}) do
        if game and game.player and game.player.class and game.saveKey then
            local pok, ok, err = pcall(RPG.Save.Write, game)
            if not pok then
                failed = failed + 1
                GLua.Warn("RPG.GameShutdown: Write threw for client " .. clientNum .. ": " .. tostring(ok))
            elseif ok then
                saved = saved + 1
            else
                failed = failed + 1
