...
function RPG.Horror.HolocronVoiceScene(player, game)
    ...
    local sequence = {
        ...
        { delay = 7100, text = "^1'You were never finding the truth.\n^1You were arriving where I sent you.'" },
    ...
    [42] = {
        ...
        examineText = function(game)
            ...
            if #log > 0 then
                ...
                -- Wording is enemy-type-neutral: reads correctly for humanoid, animal, and droid kills.
                local k = log[#log]
                lines[#lines + 1] = "The pattern matches traces from your last kill:"
                lines[#lines + 1] = (k.enemyName or "something you killed") .. "."
                lines[#lines + 1] = "Whatever moved your hands remembered it."
            else
                -- Fallback if killLog is empty (rare -- player hasn't killed since arrival).
                -- Keeps the "this is YOUR violence" hook even without a prior kill to reference.
                lines[#lines + 1] = "The blood is still wet. You have not killed"
                lines[#lines + 1] = "anything since you arrived. So whose blood is this?"
                lines[#lines + 1] = "And why does the pattern match the way YOU hold a blade?"
            end
...
In `base/glua/rpg/quest.lua`, locate `RPG.Quest.SetStage` at line 65. The function has an **outer guard** `if oldStage ~= stage then` at line 88 that closes with `end` at line 111. `RPG.Quest.ApplyStageEffects(player, questId, stage)` is called at line 110 — the LAST statement INSIDE that guard, just before the closing `end`. The bare `return true` at line 113 is OUTSIDE the guard.

**Placement is load-bearing:** the Mimic dispatcher MUST fire only on real stage changes. Place the hook **INSIDE the `if oldStage ~= stage then` block**, on a new line immediately after `RPG.Quest.ApplyStageEffects(player, questId, stage)` (line 110) and BEFORE the closing `end` of that block (line 111). Do **NOT** place it outside the guard or before the bare `return true` at line 113 — that location would re-fire the blackout on every redundant `SetStage` call (e.g., a room re-entry that re-checks quest state, or a save-load that re-applies the current stage), causing duplicate blackouts and double paranoia gain.

Insert this snippet INSIDE the guard:

```lua
        -- Phase 2A #10: dispatch Mimic blackout if this stage is one of the_mimic's tracked stages.
        -- MUST live INSIDE the `if oldStage ~= stage then` block above so it only fires on actual
        -- stage changes -- never on redundant SetStage calls that reassert the current stage.
...
After insertion, lines 110-112 read:
...
```lua
    ...
    end    -- closes `if oldStage ~= stage then` from line 88
```
...
**Healing item identification:** verified against current `data/items.lua` (2026-05-12) — the actual healing field is `healAmount`, present on 5 consumables at lines 30, 104, 160, 192, 264 (Medpac, Arkanian Medpac, Field Rations, Sliced Stim, Dxun Jungle Extract). Spec §4.1 #10 says "remove 1 medpac" — we treat that as "remove a healing consumable specifically, not any consumable." If no healing consumable is present, the removal step is **a no-op** (only the Bloodied Cloth is added). Predicate fallback to "first consumable of any kind" was rejected in plan review (Important #5): risk of removing quest-critical consumables outweighs the value of always removing something.

**Predicate ordering:** `healAmount` is checked **first** because it is the only field actually present in the current checkout. The other field names (`hpGain`, `heal`, `restoresHP`, `effect.heal`) are kept as forward-compatible fallbacks in case items get renamed in a future phase — they cost nothing and make the predicate robust to schema drift.

- [ ] **Step 1: Identify the healing-consumable predicate**

Run:
```bash
grep -n -E "healAmount|hpGain|restoresHP|heal\s*=|effect.*heal" base/glua/rpg/data/items.lua
```

**Verified against current checkout (2026-05-12):** the actual field is `healAmount`, used by 5 consumables — Medpac (30), Arkanian Medpac (50), Field Rations (15), Sliced Stim (40), Dxun Jungle Extract (35) at lines 30, 104, 160, 192, 264. Other field names (`hpGain`, `heal`, `restoresHP`, `effect.heal`) are NOT present but are kept in the predicate as forward-compatible fallbacks against future schema drift.

If at Stage 4 fire time the player happens to have no healing consumables in inventory, the removal is a no-op and only the Bloodied Cloth is added — still narratively valid (the Mimic left a trace; the player just had nothing to take).
        ...
        if inv and #inv > 0 then
            ...
            for i, itemId in ipairs(inv) do
                ...
                if def and def.type == "consumable" then
                    local heals =
                        (def.healAmount ~= nil and def.healAmount > 0)    -- actual field in current items.lua (5 matches)
                        or (def.hpGain ~= nil and def.hpGain > 0)         -- forward-compatible fallback
                        or (def.heal ~= nil and def.heal > 0)             -- forward-compatible fallback
                        or (def.restoresHP ~= nil and def.restoresHP > 0) -- forward-compatible fallback
                        or (def.effect and def.effect.type == "heal")     -- forward-compatible fallback
                    if heals then
                        removeIdx = i
                        break
                    end
...
```lua
        ...
        -- Idempotency guard: only plant once per game. Tier-4 OnStageAdvance can theoretically re-fire
        -- if the quest state machine re-enters confront_truth (edge-case reloads / save-after-blackout
        -- restore paths). Without this guard, a duplicate render of "Delivered Holocron to Zherron"
        -- reads as a bug, not horror.
        if not (game.flags and game.flags["mimic_false_journal_planted"]) then
            game.flags = game.flags or {}
            game.flags["mimic_false_journal_planted"] = true
...
Run:
```bash
grep -RIl "Phase 2A" base/glua/rpg
```

(`-R` recurses into the directory, `-I` skips binaries, `-l` prints filenames only.)
